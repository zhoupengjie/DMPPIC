...
#define ENABLE_RANDOM_MOTION

#ifdef ENABLE_RANDOM_MOTION
    // Random motion

#ifdef ENABLE_GLOBAL_DIFFUSION_COEFFICIENT
    static const Foam::scalar diffusion_coefficient = global_diffusion_coefficient;
#else
    static const Foam::scalar diffusion_coefficient = 2.299e-9; // water
#endif

    // Variance of displacement in each direction
    const Foam::scalar sqrt_variance = Foam::sqrt(2 * diffusion_coefficient * trackTime);

    // Random number generator for normal distribution
    static std::random_device rd;
    static std::mt19937 gen(rd());
    std::normal_distribution<Foam::scalar> distribution(0, sqrt_variance);

    // // Generate a random seed using process number and current time
    // auto generateRandomSeed = [](int process_num) {
    //     auto now = std::chrono::high_resolution_clock::now();
    //     auto duration = now.time_since_epoch();
    //     return static_cast<Foam::label>(duration.count()) + process_num;
    // };
    // // Generate the seed value
    // Foam::label seedValue = generateRandomSeed(0);
    // // Foam random number generator
    // Foam::Random rndGen(seedValue);
#endif
    ...
    while (ttd.keepParticle && !ttd.switchProcessor && p.stepFraction() < 1)
    {
        ...
        const vector s_diff = {distribution(gen), distribution(gen), distribution(gen)};

        // const vector s_diff = {rndGen.GaussNormal<Foam::scalar>() * sqrt_variance,
        //                        rndGen.GaussNormal<Foam::scalar>() * sqrt_variance,
        //                        rndGen.GaussNormal<Foam::scalar>() * sqrt_variance};

        // Total displacement over the time-step
        const vector s = trackTime * U_ + s_diff;
#else
        // Total displacement over the time-step
